...
int create_C(int k){ //有L[k] 生成 C[k+1]  复杂度O(KM)  k：频繁集数量 m：数据平均长度
    //初始化
    ...
    for(auto it=L_create_C_map.begin();it!=L_create_C_map.end();it++){
        ...
        //每个相等的hash 两两组成
        for(int i=0;i<L_same_hash.size();i++){       //生成C
...
int create_L(int k){ //由C[k] 生成L[k]  复杂度O(KN/32) k：频繁集数量 N：数据的数量
...
void* thread_production_C1(void* rank){  //C[1]的函数
    //传参
    pii kl=*(pii*)rank;
